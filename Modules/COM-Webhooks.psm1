...
Function Get-HPECOMWebhook {
    ...
    .EXAMPLE
    Get-HPECOMWebhook -Region us-west -Name 'Webhook event for server shutdown' -ShowRecentDeliveries
   ...
   #>
    ...
    Process {
        ...
        if ($Name -and -not $ShowRecentDeliveries) {
            $Uri = Get-COMWebhooksUri
            
        }
        elseif ($ShowRecentDeliveries) {
            
            try {
                [Array]$AllWebhooks = Invoke-HPECOMWebRequest -Method Get -Uri (Get-COMWebhooksUri) -Region $Region
                $WebhookMatch = @($AllWebhooks) | Where-Object { $_.name -ieq $Name } | Select-Object -First 1
                $WebhookID = $WebhookMatch.id
            }
            catch {
                $PSCmdlet.ThrowTerminatingError($_)
            }

            if ($Null -eq $WebhookID) {
                "[{0}] Webhook '{1}' cannot be found in the '{2}' region." -f $MyInvocation.InvocationName.ToString().ToUpper(), $Name, $Region | Write-Verbose
                return
            }
   ...
   #>
    ...
    Process {
        ...
        if (-not $WebhookID) {
            ...
            # Check if any update parameter was actually provided
            $noChanges = (-not $PSBoundParameters.ContainsKey('NewName')) -and
                         (-not $PSBoundParameters.ContainsKey('Destination')) -and
                         (-not $PSBoundParameters.ContainsKey('EventFilter')) -and
                         (-not $RetryWebhookHandshake)

            if ($noChanges) {

                if ($WhatIf) {
                    $ErrorMessage = "Webhook '{0}': No changes to apply. No update parameters were provided." -f $Name
                    Write-Warning "$ErrorMessage Cannot display API request."
                    return
                }

                $objStatus.Status = "Warning"
                $objStatus.Details = "No changes to apply. No update parameters were provided."
   ...
   #>
    ...
    Process {
        ...
        if (-not $WebhookID) {
            ...
            "[{0}] Webhook '{1}' cannot be found in the Compute Ops Management instance." -f $MyInvocation.InvocationName.ToString().ToUpper(), $Name | Write-Verbose
            ...
            if ($EventFilter -match $pattern) {
                ...
                $ErrorMessage = "Webhook '{0}': Cannot extract the resource type from the event filter configuration." -f $Name
            ...
            if (-not $Object) {
                $ErrorMessage = "Webhook '{0}': No '{1}' resource found in the '{2}' region to build the test payload." -f $Name, $typeValue, $Region
                "[{0}] {1}" -f $MyInvocation.InvocationName.ToString().ToUpper(), $ErrorMessage | Write-Verbose
                if ($WhatIf) {
                    Write-Warning "$ErrorMessage Cannot display API request."
                    return
            ...
            try {
                ...
                if ($errorResponse) {
                    ...
                    if (-not $WhatIf) {
                        $objStatus.Status = "Failed"
                        $objStatus.Details = if ($_.Exception.Message) { $_.Exception.Message } else { "Network error sending test event to '$Destination'!" }
                        $objStatus.Exception = $_.Exception.Message
                    }
                }
            }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDrb/nV7bLhdKR4
# bBUxK5od2dAcROWFmdEG7qLMInBkQqCCEfYwggVvMIIEV6ADAgECAhBI/JO0YFWU
...
# IgQgz6qBehU4gcpf1KePrKtbWQITJQ96mFD5FK8hQWqHANswDQYJKoZIhvcNAQEB
# BQAEggIAiNrV5WeJcnR4rOXajVKICOcoSwL+a53YXpyI6QnWJC0IJEawvcaXUpqR
# IESCd2oo4AgcstmuYk72bR05Qkz5/Gr8kQy/dI3a3PLy1xKTnrMzJ8sQRrag3e8w
# ljC0e9HfaNc/Z/SzqAlo1lDhfvb9fYqNd9RzAFy8eBpb/jFRokmCnq7m6qMzhYph
