            ...
            if (-not $WhatIf) {
                ...
                if ($provision_status -eq "PROVISIONED") {
                    ...
                    if ($Name -eq "Compute Ops Management") {
                        ...
                        # (loginUrl is the web UI host, not the API host — comApiUrl must be fetched separately)
                        $comApiUrl = $null
                        $_loginUri = [Uri]$LoginURLResponse.login_url
                        if ($_loginUri.Host -match '\.rugby\.hpeserver\.management$') {
                            try {
                                $indigoConfigUrl = "$($_loginUri.Scheme)://$($_loginUri.Host)/indigo-config.js"
                                "[{0}] Fetching COM API root URL from '{1}'" -f $MyInvocation.InvocationName.ToString().ToUpper(), $indigoConfigUrl | Write-Verbose
                                ...
                                if ($indigoConfig -match "REACT_APP_COM_API_ROOT_URL\s*=\s*'([^']+)'") {
                                    ...
                                    "[{0}] COM API root URL resolved to '{1}'" -f $MyInvocation.InvocationName.ToString().ToUpper(), $comApiUrl | Write-Verbose
                                }
                            }
                            catch {
                                "[{0}] Warning: could not fetch indigo-config.js — COM API URL will fall back to production pattern." -f $MyInvocation.InvocationName.ToString().ToUpper() | Write-Verbose
                            }
                        }
                        if (-not $comApiUrl) {
                            $comApiUrl = "https://$region.api.greenlake.hpe.com"
                        }
                        ...
                        if (-not $Global:HPECOMjobtemplatesUris) {
                            ...
                            # Retry loop: the freshly-provisioned COM API may return 401 for a short period
                            $maxRetries = 3
                            $retryDelay = 10
                            $attempt = 0
                            $jobTemplatesSuccess = $false
                            do {
                                $attempt++
                                try {
                                    Set-HPECOMJobTemplatesVariable -region $FirstProvisionedCOMRegion
                                    $jobTemplatesSuccess = $true
                                }
                                catch {
                                    if ($attempt -lt $maxRetries -and $_.Exception.Message -match '401|403|Forbidden|Unauthorized') {
                                        "[{0}] COM job templates retrieval failed (attempt $attempt/$maxRetries) — retrying in ${retryDelay}s..." -f $MyInvocation.InvocationName.ToString().ToUpper() | Write-Verbose
                            ...
                            } while (-not $jobTemplatesSuccess -and $attempt -lt $maxRetries)
            ...
            foreach ($item in $CollectionList) {
                ...
                if ($item.application_instance_id -eq "00000000-0000-0000-0000-000000000000" ) {
                    ...
                    if ($Service.name -eq "Data Services") {
                        $ConnectivityEndPoint = if ($Global:HPEGLauthorityURL) { "$($Global:HPEGLauthorityURL.AbsoluteUri.TrimEnd('/'))/as/token.oauth2" } else { "https://sso.common.cloud.hpe.com/as/token.oauth2" }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCHoasrC/8r9hOZ
# TdoU3Gg1/iQhFSV69vQ0SHrOho/NRKCCEfYwggVvMIIEV6ADAgECAhBI/JO0YFWU
...
# IgQgti10XdI7FO4BtJlJ7yEKWdMHwYH3LGh4GBk1hBacS/IwDQYJKoZIhvcNAQEB
# BQAEggIAATTE07v4eoeXgaOsaOmE5V6Dz/MjU8bwbtkKYXExHMrnXb0TQTLKypBa
# 7vNn1hnUNd9H2ZFEQb65QGZjgKz7nUNEOM/93oxs0cFRDig4iwGFCFF1T8wpegU3
# KBemVL96eIn4NWaEcedBlKB171p2rYh5t3hU2SMb8E330eB/wGs/Z0veSN3uDoGL
# VYuinC1lLPlmc4tSaOlNhyselN36QE8TWoUdmi6+42Q/zDnl1oyyPQqtp2SgmcrU
